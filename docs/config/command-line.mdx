...
<ParamField path="--dsn" type="string" env="DSN">
  ...
  <Tabs>
    ...
    <Tab title="ClickHouse">
      ...
      clickhouse://default:password@localhost:8123/analytics

      # TLS (port defaults to 8443 when the DSN omits it)
      clickhouse://reader:password@ch.example.com:8443/analytics?secure=true
      ```

      <Note>
        DBHub connects over ClickHouse's **HTTP interface** (8123, or 8443 for
        TLS), not the native TCP protocol. A DSN pointing at the native ports
        `9000` or `9440` is rejected at startup with the HTTP port to use
        instead, rather than failing later as an unexplained connection hang.

        The DSN must name a database. ClickHouse databases are the schema
        concept, and naming one scopes object discovery to it instead of
        fanning out across every database on the server.
      </Note>
    </Tab>
  ...
  **DSN Query Parameters:**
  ...
  |-----------|-----------|-------------|---------|
  | `sslmode` | PostgreSQL, MySQL, MariaDB, SQL Server, ClickHouse | SSL mode: `disable`, `require`, `verify-ca`, `verify-full` | `?sslmode=require` |
  | `secure` | ClickHouse | Use TLS (`true`/`false`); `sslmode` takes precedence when both are set | `?secure=true` |
