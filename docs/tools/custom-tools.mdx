...
<Tabs>
  ...
  <Tab title="ClickHouse">
    ...
    statement = "SELECT * FROM products WHERE id = {product_id: UInt32}"
    ...
    ClickHouse placeholders are named, but values are still supplied as a list:
    the Nth **distinct** placeholder, in order of first appearance, takes the
    Nth declared parameter. Declare `[[tools.parameters]]` in the order the
