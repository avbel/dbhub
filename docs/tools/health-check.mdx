...
- **Per-engine support**: Implemented for PostgreSQL, MySQL, MariaDB, and SQL Server. SQLite has no connection pool or cache-hit concept to report, and ClickHouse is not implemented yet, so `health_check` returns an `UNSUPPORTED` error on both
